...
- Importing the package seizes the root logger and creates `output.log` in the working
  directory (R5).
- Cyclic references raise `RecursionError` despite being advertised as supported (R6).
- `from_dict` resolves types against the framework module's `globals()`, so polymorphic
  deserialization of user types does not work (R7).
- A container with caching enabled still serves a stale `to_dict` result after one of its
  items is mutated in place, and the cached mapping is handed out live (R11).
- `_type_cache` is keyed by type *name* and shared across the hierarchy, so two modules
  defining a class with the same name collide (R1).
